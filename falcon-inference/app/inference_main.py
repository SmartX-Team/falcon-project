# falcon-inference/app/inference_main.py
import os
import json
import logging
import signal
...
INPUT_TOPIC = os.environ.get('INPUT_TOPIC', 'fused_input_for_inference') # wrapper의 OUTPUT_KAFKA_TOPIC과 일치해야 함
OUTPUT_TOPIC_INFERENCE = os.environ.get('OUTPUT_TOPIC_INFERENCE', 'inference_results') # 추론 결과를 발행할 토픽
...
X_CORRECTION_OFFSET = float(os.environ.get('X_CORRECTION_OFFSET', 1.0)) # 미터 단위
DEPTH_OFFSET_FACTOR = float(os.environ.get('DEPTH_OFFSET_FACTOR', -1.0)) # 원본 코드의 -1 보정치
...
kafka_producer = None
# app_config 객체가 없으므로 KafkaProducer 설정은 환경변수나 기본값 직접 사용
...
def consume_messages():
    ...
    if not yolo_model or not unidepth_session: # 두 모델 중 하나라도 로드 안되면 실행 불가
    ...
    try:
        ...
        kafka_producer = None # 프로듀서 없이 진행 가능하도록 설정
        ...
        try:
            for message in consumer:
                ...
                try:
                    ...
                    person_locations = [] # 추론된 사람 위치 정보 저장 리스트
                    ...
                    if kafka_producer:
                        try:
                            kafka_producer.send(OUTPUT_TOPIC_INFERENCE, value=inference_output)
                    ...
                    logger.error(f"Error processing message for camera_id '{camera_id}': {e_proc}", exc_info=True)

        except Exception as e_consumer_loop:
            logger.error(f"Error in Kafka consumer loop: {e_consumer_loop}", exc_info=True)
